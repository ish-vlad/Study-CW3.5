...
public class ChoiseActivity extends Activity implements onGroupSelect, onObjectSelect {
	...
	public static final String EXTRA_CHOISE = TAG + "_choise";
	public static final String EXTRA_GROUP = TAG + "_group";
	public static final String EXTRA_LECTURE = TAG + "_lecture";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			mOpt = (Choise)savedInstanceState.getSerializable(EXTRA_CHOISE);
			...
			if (savedInstanceState.getInt(EXTRA_GROUP) != -1) {
				...
				if (savedInstanceState.getInt(EXTRA_OBJECT) != -1) {
					...
					if (savedInstanceState.getInt(EXTRA_LECTURE) != -1) {
						mLectureId = savedInstanceState.getInt(EXTRA_LECTURE);
						checkPeople();
					}
					else {
			...
			mOpt = (Choise)getIntent().getExtras().getSerializable(EXTRA_CHOISE);
		}
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		outState.putSerializable(EXTRA_CHOISE, mOpt);
		outState.putInt(EXTRA_LECTURE, mLectureId);
		super.onSaveInstanceState(outState);
	}
	
	public enum Choise { VIEW, LECT, LAB }

	@Override
	public void groupSelect(int groupId) {
		mGroupId = groupId;
		mObjectId = -1;
		ObjectHelper objHelp = new ObjectHelper(getBaseContext());
